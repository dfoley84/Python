...
def SourceRDSInstance():    
    command = [
    'mysqldump --user="%s" --password="%s" -h %s ebdb > /tmp/%s.sql' % (SourceUser, Sourcepwd, SourceRDS, customer)
    ]
    try:
...
def DropTable():
    ...
    with open_tunnel(
        ...
        for sql in sql:
            print("running command: {}".format(sql))
            cur.execute(sql)
            print(cur)
        cur.close()
...
def RestoreDB():
    ...
    command = [
    'mysql --user=%s --password="%s" -h %s ebdb < /tmp/%s.sql' % (TargetUser, Targetpwd, TargetRDS, customer)
    ]
    try:
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
...
#Remove SQLDump File From EC2 
def RemoveDumpFile():
    command = ['sudo rm -rf /tmp/%s.sql' % (customer) ]
    try:
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(REMOTE_HOST, username='ec2-user', key_filename='./t.pem')
...
if __name__ == '__main__':
    ...
    REMOTE_HOST = #EC2 Public DNS Name
